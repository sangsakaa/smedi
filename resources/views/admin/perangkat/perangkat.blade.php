    ...
    <div class="inline-flex overflow-hidden mb-4 w-full bg-white rounded-lg shadow-md">
        ...
        <div class="px-4 py-2 -mx-3">
            <div class="mx-3 font-semibold uppercase text-green-800">
                <form action="/perangkat" method="get">
                    <input type="text" name="cari" value="{{ request('cari') }}"
                        class="  border-green-800 border text-green-800 rounded-md py-1 px-4" placeholder=" Cari ..">
                    <button type="submit" class=" border  bg-green-800 py-1 px-2 rounded-md text-white">
                        Cari</button>
                </form>
            </div>
        </div>
        ...
        <div class=" w-full px-2 py-4 ">
            <div class="mx-3">
                <div class=" bg-white rounded-lg shadow-xs">
                    <div>
                        <a href="/perangkat/create"><button
                                class=" inline-block  justify-items-end  bg-green-800 px-2 py-1 text-white rounded-md"><svg
                            ...
                            <div class="overflow-x-auto w-full">
                                <table class="w-full whitespace-no-wrap" id="#myTable">
                                    ...
                                    <tbody class="bg-white divide-y capitalize">
                                        @if($dataPerangkat->count())
                                        @foreach( $dataPerangkat as $perang)
                                        <tr class="text-gray-700">
                                            <td class="px-4 py-2 text-sm ">
                                                {{ $loop->iteration }}
                                            </td>
                                            <td class="px-4 py-2 text-sm ">
                                                {{ $perang->nama_perangkat }}
                                            </td>
                                            <td class="px-4 py-2 text-sm ">
                                                {{ $perang->jenis_kelamin }}
                                            </td>
                                            <td class="px-4 py-2 text-sm ">
                                                {{ $perang->no_hp }}
                                            </td>
                                            ...
                                            <td class=" px-4 py-2 text-sm">
                                                <div class=" flex">
                                                    <div class="flex">
                                                        <form action="/perangkat/{{$perang->id}}" method="post">
                                                            @csrf
                                                            @method('delete')
                                                    ...
                                                    <div class=" flex">
                                                        <a href="/perangkat/{{$perang->id}}/edit">
